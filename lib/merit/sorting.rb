...
module Merit
  module Sorting
    ...
    #
    # sort_key  - A block which returns the key by which each item in the
    #            collection is sorted.
    # selector - A block which selects which items are variable (when returning
    #            true) or fixed (when returning false). This allows for
    #            significantly better performance in `Variable` when many items
    #            have non-variable pricing. Note that omitting the `selector`
    #            will cause all collections created in `Sorting.for_collection`
    #            to be `Fixed`.
    ...
    #
    # Note that because collections store "seen" items in a hash, the
    # performance of `initialize` and `insert` is quite sensitive to the
    # performance of `Item#hash` (where `Item` is the class of an object stored
    # in the collection).
    class Unsorted
      extend Forwardable
    ...
    #
    # Sorting collections every hour is enormously slower than doing so only
    # once; use this with care!
    #
    # `Variable` partitions the items into two separate sub-arrays: one for
    # containing variably-priced items, for which the selector returns true, and
    # another containing items with a constant price. This allows the collection
    # of fixed-price items to be sorted only once.
    #
    # In each call to `at_point`, the collection is emptied and repopulated with
    # the fixed-price items. The variable-priced items are then inserted into
    # the collection using a binary search.
    #
    # In a typical case, this results in much improved performance over sorting
    # the entire collection every time. With 40 items, 5 of which have variably
    # pricing, calling `at_point` 8760 times, once for each hour in the year:
    #
    ...
    #
    # When the Variable contains only varible-priced items, binary search insert
    # performs roughly the same as simply resorting the whole collection. With
    # 5 variable items:
    #
    #   * Resorting everything: 64.7 i/s
    #   * Partition and binary search insert: 69.5 i/s
    #
    # Variable will fall back to simple `sort_by` when it contains 33% or more
    # variable-priced items as this offers better performance in that
    #
    # It is essential that the config.sort_key return consistently ordered
    # values for the fixed-price items. A sort_key which ordered the fixed-price
    # items incorrectly (for example `->(*) { rand }`) will result in the binary
    # search inserting items in incorrect positions.
    class Variable < Fixed
      # If no default selector is provided (to partition the collection into
      # fixed and variable priced items) then all items are considered variable.
      DEFAULT_SELECTOR = ->(_) { true }

      # Always use binary search insert when the collection has a small number
      # of items.
      SIMPLE_SORT_LENGTH_THRESHOLD = 10

      # When there are many more variable items than fixed, binary search insert
      # becomes significantly slower than using `sort_by` on the whole
      # collection. This seems to occur when there is one or two variables for
      # every fixed; this varies slightly depending on the size of the
      # collection. The threshold determines at what point Variable switches to
      # using `sort_by`.
      ...
      # When the collection contains only variable-priced items, and they number
      # 6 or greater, a simple sort_by outperforms binary search insert.
